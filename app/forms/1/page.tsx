...
export default function Form1Page() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!checkFormAccess(conf, "form_1")) {
        router.replace("/hub");
        return;
      }
  ...
  if (loading || !user || !config) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm font-medium text-slate-500">กำลังโหลดแบบประเมิน...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50/40 via-slate-50 to-indigo-50/30 pb-16">
      {/* Top Navigation Bar */}
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200/80 px-4 py-3">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <Link
            href="/hub"
            className="inline-flex items-center gap-2 text-xs sm:text-sm font-semibold text-slate-600 hover:text-blue-600 bg-slate-100 hover:bg-blue-50 px-3 py-1.5 rounded-xl transition-all border border-slate-200 hover:border-blue-200"
          >
