...
export default function Form3Page() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!checkFormAccess(conf, "form_3")) {
        router.replace("/hub");
        return;
      }
      ...
      if (batchCompleted.includes("form_3")) {
        // Only editCount === 0 is editable; -1 means there is no editable record.
        const editCount = await getEditCount(user.uid, "form_3", conf.currentBatch);
        if (editCount !== 0) {
          router.replace("/hub");
          return;
        }
        const answers = await getExistingSubmission(user.uid, "form_3", conf.currentBatch);
        if (cancelled) return;
        setIsEditing(true);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-teal-50/40 via-slate-50 to-emerald-50/30 pb-16">
      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200/80 px-4 py-3">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
            <span className={`w-2 h-2 rounded-full inline-block animate-pulse ${isEditing ? "bg-amber-500" : "bg-teal-500"}`} />
            <span>{isEditing ? "โหมดแก้ไข" : "โหมดประเมินพยาบาลวิสัญญี"}</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 pt-6">
        <Form3
          userId={user.uid}
          departments={config.form3Departments}
