  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="w-full flex items-center justify-between p-3.5 sm:p-5 text-left bg-gradient-to-r from-teal-50/90 via-emerald-50/40 to-slate-50 hover:bg-teal-50 transition-colors gap-2 sm:gap-3"
      >
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          <div
            className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl flex items-center justify-center font-bold text-xs sm:text-sm shrink-0 shadow-sm ${hasErrors
          ...
          <div className="min-w-0">
            ...
            <span className="text-[11px] sm:text-xs text-teal-700 font-medium flex items-center gap-1 mt-0.5">
              <Users className="w-3.5 h-3.5 shrink-0" />
              <span className="truncate">{staff.length} ท่าน {allowSkip ? "(เลือก เคย/ไม่เคย)" : "(จำเป็นทุกท่าน)"}</span>
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          {hasErrors ? (
            <span className="text-[10px] sm:text-xs font-bold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-rose-100 text-rose-700 border border-rose-200 shrink-0 flex items-center gap-1">
              <AlertTriangle className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-rose-600" />
              ยังไม่ครบ
            </span>
          ) : isAllFilled ? (
            <span className="text-[10px] sm:text-xs font-bold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200 shrink-0 flex items-center gap-1">
              <CheckCircle2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-600" />
              <span className="hidden xs:inline">ครบแล้ว</span> ({filledCount}/{staff.length})
            </span>
          ) : (
            <span className="text-[10px] sm:text-xs font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-slate-100 text-slate-600 border border-slate-200 shrink-0">
              {filledCount}/{staff.length}
            </span>
          )}
          <div className="p-0.5 sm:p-1 rounded-full text-slate-400 hover:text-slate-600">
...
export function Form3({
  ...
  const { lastSavedAt, clearDraft, isSaving } = useDraftAutoSave<Form3Values>({
    formId: "form_3",
    batchId,
    userId,
    getValues,
